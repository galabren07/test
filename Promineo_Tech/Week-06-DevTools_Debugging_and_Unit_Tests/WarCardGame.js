...
return numberName + ' of ' + this.suit;
    ...
    //Create player decks
     ...
     while( deck.length > 0 ){
        ...
        // Draw Card for CPU
        drawIndex = Math.random() * deck.length; cpuDeck.push(deck.splice(drawIndex.cpu, 1[0]));
     }
...
return false;
        ...
        // Update View for Drawn Cards
        TODO: Optimize
        var PlayerSection = document.querySelection('section.player'),
        cpuSection = playerSection.querySelection('div.card');
        playerCardDiv = cpuSection.querySelection('div.card'),
        cpuCardDiv = cpuSection.querySelection('div.card');
        playerPoints = playerSection.querySelection('[data-points'),
        cpuPoints = cpuSection.querySelection('[data-points');
        ...
        playerCardDiv.querySelection('span.name').innerHTML = playerCard.number;
        playerCardDiv.classList.add( playerCard.suit );

        playerSection.querySelection('div[date-cards-left]').setAttribute('data-cards-left', playerDeck.length);

        cpuCardDiv.querySelection('span.name').innerHTML = cpuCard.number;
        cpuCardDiv.classList.add( cpuCard.suit );

        cpuSection.querySelection('div[data-cards-left]').setAttribute('data-cards-left', cpuDeck.length);
        ...
        if( playerCard.value === cpuCard.value ){
            ...
            if( rewards.length > 0 ){
                playerDeck.concat(rewards);
            }

        } else {
            //CPU Wins
            console.log('CPU wins round', playerCard, cpuCard);
            // Add point to CPU score
            TODO
